...
def create_odd(apps, mercado, tipo, nombre, valor):
...
def forwards(apps, schema_editor):
    ...
    for evento in Evento.objects.select_related('equipo_local', 'equipo_visitante').filter(activo=True):
        ...
        evento_nombre = f'{local} vs {visitante}'
        if evento.nombre != evento_nombre:
        ...
        for tipo, nombre, selecciones in specs:
            mercado, _ = Mercado.objects.get_or_create(
                evento=evento,
                tipo=tipo,
                defaults={'nombre': nombre, 'activo': True, 'suspendido': False},
            )
            for sel_tipo, sel_nombre, valor in selecciones:
                create_odd(apps, mercado, sel_tipo, sel_nombre, valor)
